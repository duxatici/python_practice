...
class TestMoney:
    @pytest.fixture
    def money1(self):
        return Money(value=Decimal(1), currency=rub)

    @pytest.fixture
    def money2(self):
        return Money(value=Decimal(2), currency=rub)

    @pytest.fixture
    def money3(self):
        return Money(value=Decimal(3), currency=rub)
    ...
    def test_other_currency(self, money1, money2, money3):
        with pytest.raises(NotComparisonException):
            Money(value=Decimal(1), currency=rub) + Money(
...
class TestWallet:
    @pytest.fixture
    def money(self):
        return Money(value=Decimal(500), currency=rub)
    ...
    def test_get__empty(self, wallet):
        assert wallet[usd] == Money(value=Decimal(0), currency=usd)
    ...
    def test_add(self, wallet):
        wallet.add(Money(value=Decimal(100), currency=rub)).add(
            Money(value=Decimal(200), currency=rub)
        )
        assert wallet[rub] == Money(value=Decimal(800), currency=rub)

    def test_sub(self, wallet):
        wallet.sub(Money(value=Decimal(100), currency=rub)).sub(
            Money(value=Decimal(200), currency=rub)
        )
        assert wallet[rub] == Money(value=Decimal(200), currency=rub)

    def test_sub__negative(self, wallet):
        with pytest.raises(NegativeValueException):
            wallet.sub(Money(value=Decimal(math.inf), currency=rub))
